...
func (dv *dvLatency) handleCreateDV(obj interface{}) {
	...
	podLabels := dataVolume.GetLabels()
		...
		for _, c := range dataVolume.Status.Conditions {
			...
			case cdiv1beta1.DataVolumeBound:
				if dvm.dvBound.IsZero() {
					log.Infof("Updated bound time for dataVolume [%s]", dataVolume.Name)
					dvm.dvBound = c.LastTransitionTime.Time.UTC()
				}
			case cdiv1beta1.DataVolumeRunning:
				if dvm.dvRunning.IsZero() {
					log.Infof("Updated running time for dataVolume [%s]", dataVolume.Name)
...
func (dv *dvLatency) calcQuantiles() {
	...
	dv.latencyQuantiles = calculateQuantiles(dv.uuid, dv.jobConfig.Name, dv.metadata, dv.normLatencies, getLatency, podLatencyQuantilesMeasurement)
}
