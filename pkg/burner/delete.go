...
func (ex *Executor) RunDeleteJob() {
	var objectFinalizer ObjectFinalizer
	if ex.Job.WaitForDeletion {
		objectFinalizer = verifyDelete
	}
	ex.runJob(ex.deleteHandler, objectFinalizer)
}

func (ex *Executor) deleteHandler(obj object, item unstructured.Unstructured, iteration int, wg *sync.WaitGroup) {
	defer wg.Done()
